...
void ThreadIRCSeed(void* parg)
{
    ...
    RenameThread("LuminosityCoin-ircseed");
...
void ThreadIRCSeed2(void* parg)
{
    ...
    while (!fShutdown)
    {
        ...
        if (fTestNet) {
            Send(hSocket, "JOIN #LuminosityCoinTEST2\r");
            Send(hSocket, "WHO #LuminosityCoinTEST2\r");
        } else {
            // randomly join #LuminosityCoin00-#LuminosityCoin05
            // int channel_number = GetRandInt(5);

            // Channel number is always 0 for initial release
            int channel_number = 0;
            Send(hSocket, strprintf("JOIN #LuminosityCoin%02d\r", channel_number).c_str());
            Send(hSocket, strprintf("WHO #LuminosityCoin%02d\r", channel_number).c_str());
        }
